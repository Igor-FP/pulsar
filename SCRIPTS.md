...
*Проверка индексов*: `wsl bash -lc "ls /usr/share/astrometry"` показывает установленные индексы; в режиме `-v` видно, какие индексы solve-field перебирает. Для очень узких/широких полей может понадобиться докачать соответствующий индекс astrometry.net.
...
1. **Выравнивание по референсу** (с `--ref`) — приводит все кадры к единой системе координат референсного кадра
2. **Коррекция хроматизма** (без `--ref`) — выравнивает R и B каналы по G внутри RGB-файла. Исправляет атмосферную рефракцию и хроматическую аберрацию

**Синтаксис**:
```
staralign input_spec output_spec --ref reference.fit [options]
staralign input_spec output_spec                     (хроматический режим)
```
...
|----------|-----------|----------|
| `--ref FILE` | — | Референсный кадр (без него — режим коррекции хроматизма) |
...
:: Коррекция хроматизма в RGB-файле
staralign color.fit corrected.fit

:: Батч коррекции хроматизма
