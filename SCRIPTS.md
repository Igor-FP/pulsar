...
| **blend.py** | Совмещение двух изображений по маске непрозрачности: out = source*(1-m) + operand*m; --mtf, --invert |
...
- `mask` - полутоновая маска непрозрачности (FITS-файл или последовательность по числу кадров). НЕ числовая константа.
- `operand` - изображение, подмешиваемое там, где маска светлая: FITS-файл, последовательность или числовая константа (ровный уровень)
- `--mtf [K]` - применить MTF (функцию передачи полутонов) к маске в [0,1] ДО совмещения и ДО `--invert`. K - баланс полутонов (0<K<1), как в mtf.py: K<0.5 осветляет маску (больше операнда), K>0.5 затемняет (больше исходника). При отсутствии K по умолчанию 0.25.
- `--invert` - использовать инвертированную маску (m -> 1 - m); применяется ПОСЛЕ `--mtf`
...
:: Притушить замаскированные области к 0 (операнд = константа 0)
...
`source` и `operand` должны совпадать по форме и шкале данных; выход сохраняет тип и заголовок `source`. Поддерживаются 2D и 3-канальные цветные изображения (моно-маска транслируется на все каналы). Если операнд - числовая константа, задавайте K для `--mtf` явно (например `--mtf 0.3`), чтобы константа не была принята за K.
