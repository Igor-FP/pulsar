...
| **backflat.py** | Выравнивание RGB-фона по ФМФ: беззвёздный кадр, ручная маска, диффузионное заполнение |
...
| **flip.py** | Зеркальный флип по осям: flip-Y (`Ynew=H-Y-1`, верх/низ), flip-X (`Xnew=W-X-1`, лево/право) |
...
| **makemask.py** | Создание масок: ЧБ, обрезка чёрной/белой точки (перцентиль или абсолют) со стретчем, наращивание/сужение, инверсия |
| **blend.py** | Совмещение двух изображений по маске непрозрачности (или ровной непрозрачности `-o N`): out = source*(1-m) + operand*m; `-o`/`--opacity` ослабляет финальную непрозрачность, --mtf, --invert |
...
| **cometalign.py** | Выравнивание серии по ядру кометы (интерактивная отметка первого/последнего кадра + сдвиг по времени) |
...
- `--bgcomp [P]` — Компенсация уровня фона (только вместе с `--continuum`). Вычитание `K*operand` понижает фон примерно на `K*фон(operand)`; эта опция добавляет его обратно, чтобы фон результата остался таким же, как у входного кадра до вычитания. Добавляемая величина — P-й перцентиль МАСШТАБИРОВАННОГО вычитаемого (`K*operand`), т.е. его уровень фона, вычисляется ТОЛЬКО по ненулевым пикселям (нулевые/no-data поля выровненных кадров игнорируются — иначе при доле нулей > P% фон занизился бы до 0 и компенсация не сработала бы). P — фактор детекции фона (по умолчанию 10, диапазон 0..100). Складывается с `offset`.
- `--float` — Записывать результат как float32 вместо обрезки под целочисленный тип входа. Иначе целочисленный выход обрезает значения ниже 0 (например, остатки континуума → 0 для беззнаковых типов) или выше максимума типа (уход в другой тип потерял бы большие положительные значения); float32 сохраняет и те, и другие. Float-вход сохраняет свой тип. По умолчанию выключено (выход сохраняет тип входа — прежнее поведение).
...
**Формула с --continuum --bgcomp**: `result = input - K * operand + bg + offset`, где `bg` — P-й перцентиль `K*operand` по ненулевым пикселям (P по умолчанию 10), возвращающий фон к исходному уровню.
...
sub ha.fit hae.fit red.fit --continuum --bgcomp           # фон сохранён (перцентиль 10%)
sub ha.fit hae.fit red.fit --continuum --bgcomp 5         # фактор детекции фона 5%
sub ha.fit hae.fit red.fit --continuum --float            # float32: отрицательные остатки не теряются
```
...
**Назначение**: Выравнивание фона RGB-композиции методом ФМФ (Фактическая Модель Фона, MDB — Masked Diffusion Background). Фон строится из беззвёздного изображения круглыми медианами и Gaussian-фильтрами; области протяжённых объектов исключаются маской и заполняются диффузией от фиксированных берегов. Полиномиальная аппроксимация и сетка опорных точек не используются.
...
Главный путь — `--starless`: заранее подготовленный беззвёздный FITS с теми же размерами, регистрацией (кроме возможного Y-флипа) и единицами яркости. Вход строго RGB `(3, H, W)`. Поддерживаются типы FITS int/uint и float; float не обязан лежать в `[0,1]`. Файлы раскрываются через `batch_utils`: одиночный, `=файл`, последовательность, маска, список. Для серии беззвёздных кадров требуется столько же, сколько исходных; одна маска может применяться ко всей серии.

**Сохранение:** при успешном завершении всегда записываются три файла: выровненный кадр `output_spec`, модель фона `background.fit` и маска `back_mask.fit`. По умолчанию фон и маска находятся рядом с одиночным выходным кадром; для серии — в отдельном каталоге `<имя_выхода_без_расширения>_backflat/` каждого результата. `--out-back SPEC` и `--out-mask SPEC` меняют пути сохранения. Для серии применяются стандартные выходные шаблоны с номером или каталоги; явные относительные пути отсчитываются от текущего рабочего каталога. `--mask SPEC` задаёт только входную маску.

Если любой выходной файл уже существует, весь запуск останавливается **до обработки и открытия окна** с перечислением файлов. Для разрешения перезаписи нужен `--overwrite` или `-y`, в том числе при повторном открытии существующего архива маски. Повторные сохранения маски, уже записанной в текущем сеансе, разрешены.

**Результат и ФМФ — float32**, вычисления — float64. RGB-модель вычитается поканально, затем всем каналам прибавляется один нейтральный уровень — среднее трёх средних ФМФ:
```text
K = (mean(background[R]) + mean(background[G]) + mean(background[B])) / 3
out[c] = image[c] - background[c] + K
```
Так постоянный цветовой сдвиг модели не возвращается обратно. Сохраняется общее среднее по RGB, а не отдельные средние каналов. Нейтральность остаточного реального фона зависит от точности ФМФ; шум и ошибки модели не устраняются добавлением K.
Отрицательные значения сохраняются. Нули обрабатываются как значения изображения, не как универсальная маска отсутствия данных. NaN/Inf заменяются нулями при вводе/приведении выхода с предупреждением; нормальные значения не ограничиваются диапазоном `[0,1]`. Заголовок сохраняется, устаревшие поля масштаба/контрольных сумм обновляются, параметры, источник маски, библиотека медианы и отсутствие квантования записываются в HISTORY. Исходное изображение нельзя перезаписать выходом этой команды.

**Параметры обработки**. Все пиксельные размеры заданы для диагонали 7515 px (примерно 6248×4176) и автоматически умножаются на `hypot(W,H)/7515`. Медиана имеет круглую апертуру указанного диаметра; Gaussian-диаметр — **FWHM**, `sigma = FWHM / sqrt(8*ln(2))`.

| Параметр | Умолчание | Значение |
|---|---:|---|
| `--median1 D` | 40 | Первая круглая медиана беззвёздного кадра |
| `--edge F` | 0.025 | Ширина зеркальных полос как доля диагонали; сначала левая/правая, затем верхняя/нижняя |
| `--blur1 D` | 40 | Gaussian FWHM после исправления краёв |
| `--median2 D` | 60 | Повторная круглая медиана |
| `--median-mode MODE` | fast | `fast` — DIPlib; `exact` — контрольный SciPy. Оба в float64 без квантования |
| `--median2-scale N` | 4 | Максимальное уменьшение второй медианы по каждой оси: 1, 2 или 4 |
| `--diffusion-scale N` | 4 | Максимальное уменьшение заполнения: 1, 2 или 4; 1 — полный размер |
| `--lake-mask D` | 100 | Gaussian FWHM маски для смешивания озера, затем автоуровни 0..1 |
| `--lake-blur D` | 250 | Gaussian FWHM заполненной области |
| `--blur D` | 60 | Общий Gaussian FWHM |
| `--center-plateau F` | 0.5 | Порог расстояния до ближайшей стороны как доля максимального расстояния |
| `--center-mask D` | 150 | Gaussian FWHM центральной маски |
| `--center-blur D` | 225 | Дополнительный Gaussian FWHM центра |
| `--margin D` | 1% диагонали | Запас вокруг нарисованной маски; явное D задаётся в опорных пикселях |
| `--mask SPEC` | архив, если есть | Загрузить моно-маску: любое положительное значение исключает пиксель |
| `--no-gui` | выключено | Обработать без окна, обязательно с существующей маской |
| `--out-back SPEC` | background.fit | Изменить путь сохраняемой модели фона |
| `--out-mask SPEC` | back_mask.fit | Изменить путь сохраняемой маски |
| `-y`, `--overwrite` | выключено | Разрешить перезапись существующих выходных файлов |
| `--starless SPEC` | — | Готовые беззвёздные кадры; главный путь, Y-ориентация проверяется |
| `--starless-flip-y` | выключено | Принудительно перевернуть беззвёздный кадр по Y вместо измерения |
| `--starless-no-flip` | выключено | Сохранить порядок строк без проверки ориентации |
| `--sxt` / `--starnet` | выключено | Альтернатива `--starless`: внешний движок, выбрать ровно один путь |
| `--sxt-exe FILE` / `--starnet-exe FILE` | поиск в PATH | Путь к выбранному внешнему движку |
| `--cache-dir DIR` | .backflat-cache рядом с маской | Временный кэш; очистка после завершения запуска |
| `-h`, `--help` | — | Полная встроенная справка |

**Точность и ускорение:** fast использует [DIPlib](https://diplib.org/diplib-docs/nonlinear.html) (Apache 2.0), exact — SciPy. Первая круглая медиана всегда полноразмерная. Вторая в fast по умолчанию работает на изображении, уменьшенном максимум в 4 раза по каждой оси после подготовительного Gaussian; блоки усредняются в float64, результат восстанавливается билинейно по центрам пикселей. Интенсивности не квантуются и не обрезаются. Итоговое сохранение float32 — отдельное округление.

Ускоренное заполнение также использует уменьшение максимум в 4 раза. В усреднение попадает только известный фон вне маски, включая частично покрытые блоки: содержимое дыр исключается до уменьшения. После интерполяции исходные полноразмерные берега восстанавливаются точно. Фильтры озера, общего фона и центра работают в полном разрешении. Уменьшение автоматически ограничивается размером ядра: не менее 2 пикселей на sigma покрытия; для второй медианы — также не менее 12 пикселей на диаметр и 2 пикселей на sigma предварительного Gaussian. Маленькие кадры/ядра обрабатываются с меньшим уменьшением или без него.

Расчёт без пространственного уменьшения: `--median2-scale 1 --diffusion-scale 1`. Значение 2 — промежуточный вариант. `--median-mode exact` принудительно отключает уменьшение второй медианы и использует SciPy; масштаб заполнения задаётся отдельно. На предоставленном RGB 6248×4176 новые умолчания сократили вычисления примерно с 599 до 74 с; отличие ФМФ: RMS 0,003, максимум 0,033 в исходных единицах яркости. Это результаты данного кадра и оборудования.

**Журнал:** консоль показывает время начала и длительность операций: чтение FITS, обе медианы, зеркальные полосы, Gaussian, заполнение, маску, смешивание, вычитание и сохранение. Длинные медианы и заполнение примерно каждые 5 с сообщают строки/канал или итерацию. В окне текущая операция и прошедшее время видны во время расчёта. Вывод ASCII, пригоден для перенаправления в файл.

**Диффузия**: содержимое дыр не используется. На каждой итерации Gaussian FWHM 20 распространяет покрытие, Gaussian FWHM 80 — поддержанные RGB-значения с нормировкой на покрытие; альфа уплотняется вдвое. Берега восстанавливаются на каждой итерации. Итерации заканчиваются после заполнения дыр; это последовательное Gaussian-заполнение, а не заявление о точном решении уравнения Лапласа. Затем следуют размытие озера FWHM 250 по размытой маске, общий блур и обязательный блур центра. Маска объектов размывается с FWHM 100, затем линейно растягивается по своим min/max в 0..1 без отсечения процентилей: `(mask-min)/(max-min)`. Пустая маска остаётся нулевой; постоянная маска не растягивает численный шум. Центральная маска строится от расстояния до сторон; после блура её чёрная точка корректируется, на границе она равна нулю. Полностью замаскированный кадр отклоняется: источника фона нет. Пустая маска допустима.

**Интерактив**: чистый pygame, подготовленный кадр вписан в окно. Уменьшение интенсивностей — усреднением блоков; MTF меняет только отображение. Маска хранится в полном разрешении, показывается белым с прозрачностью 20%, запас — отдельным контуром.

| Управление | Действие |
|---|---|
| ЛКМ / ПКМ | Рисовать / стирать круглой кистью |
| Колесо / Shift+колесо | Диаметр кисти ±1 / ±10 пикселей изображения |
| `+` / `-`, средняя кнопка | Зум под курсором / панорамирование |
| Home / End | MTF ярче / темнее, шаг ×1.4 |
| M, Ctrl+Z | Показать маску / отменить мазок |
| Tab | Разметка / параметры |
| `1`, `2`, `3`, `4` | Исходный, подготовленный, ФМФ, результат |
| Клик по параметру | Ввести число; Enter подтвердить, Esc отменить ввод |
| Стрелки вверх/вниз, влево/вправо | Выбрать / изменить параметр; Shift — мелкий шаг |
| Apply, B, Enter | Сохранить маску и пересчитать предпросмотр; итоговые изображения записываются при выходе |
| S | Сохранить маску, не закрывая окно |
| Q, Esc, закрытие | Применить параметры, сохранить маску, модель фона и выровненный кадр, выйти |

**Архив и кэш**: маска сохраняется всегда по пути `--out-mask` или по описанному выше умолчанию `back_mask.fit`. Основной HDU — uint16 `0/65535`, совместимый с `makemask`/`blend`; расширение `RAWMASK` в том же формате uint16 хранит исходные мазки, `BFMARGIN` — применённый запас. При повторном открытии запас не накапливается. Если есть `RAWMASK`, загружается именно он: редактирование только Primary HDU внешним инструментом не меняет исходную разметку. Обычная маска без `RAWMASK` читается из Primary HDU. Маска бинарная: конечное значение больше нуля исключает пиксель; пустая маска допустима, полностью закрытый кадр отклоняется. `--mask` задаёт исходную маску; архив всё равно создаётся. Существующий архив по выбранному выходному пути подхватывается автоматически при разрешённой перезаписи (`-y`); при отсутствии явного `--margin` восстанавливается сохранённый запас. Для новой разметки без загрузки архива задайте новый путь `--out-mask` или другой каталог результата.

На диске временно кэшируется **только внешний этап удаления звёзд**, по хешу исходного файла и идентичности executable. Каталог по умолчанию `.backflat-cache` рядом с выходной маской, переопределяется `--cache-dir DIR`. После закрытия окна и сохранения результатов, а также при ошибке или Ctrl+C, служебные FITS кэша удаляются, затем удаляется пустой каталог. В серии очистка выполняется после всего запуска. С `--starless` новый кэш не создаётся, но старый кэш по выбранному пути тоже очищается. Проверка аргументов/существующих выходов завершается до обработки и кэш не трогает.

Удаляются только распознанные файлы Backflat: имя из 64 hex-символов и отметка `backflat starless cache:` в HISTORY. Входы, выходы, посторонние файлы, подкаталоги и ссылки не удаляются; при их наличии каталог остаётся. Ошибка доступа при очистке выводится явно и делает код завершения ненулевым. Принудительное убийство процесса не позволяет гарантировать очистку. Следующий запуск `--sxt`/`--starnet` снова удалит звёзды; для постоянного беззвёздного исходника используйте `--starless FILE`.

Внутри текущего сеанса беззвёздный кадр и последняя подготовка остаются в RAM: изменения параметров не запускают внешний движок заново, изменения стадии C не повторяют стадию A.

**Ориентация беззвёздного кадра:** все три источника (`--starless`, `--sxt`, `--starnet`), включая чтение кэша, проходят одну проверку до подготовки, загрузки маски и открытия окна. Сравниваются варианты как есть и отражённый по Y. Измерение использует усреднённую RGB-копию с длинной стороной до 1024 пикселей, удаление линейного градиента, high-pass на двух масштабах и ограничение влияния ярких звёзд только в измерительной копии. На обоих масштабах должен выиграть один вариант с разницей корреляций не менее 0,05. При уверенном Y-флипе строки разворачиваются и это пишется в лог; правильная ориентация проходит без отдельного сообщения. Решение, метод и корреляции попадают в HISTORY обоих выходных изображений.

Симметричное/почти плоское поле, слишком маленькое превью (меньше 32 пикселей по короткой стороне) или противоречивые оценки останавливают запуск с объяснением. Взаимоисключающие `--starless-flip-y` и `--starless-no-flip` принудительно задают Y-флип или сохранение строк без проверки. Они относятся к поданному файлу или исходному результату движка: отдельного автоматического флипа SxT больше нет. Маска остаётся в координатах исходного кадра. Это проверка только Y-отражения, не регистрация: X-флип, сдвиг, поворот и масштаб изображения не исправляются; исходник и беззвёздный кадр должны иметь одинаковые размеры и единицы яркости.

В серии проверяется каждая пара; смена необходимой ориентации или неопределённость останавливает всю серию. Результаты ранее завершённых кадров остаются. Порог выбран по NGC 300 и контрольным искажениям: корреляции 0,303 против 0,015 и 0,298 против 0,055; проверка полного RGB 6248×4176 заняла около 0,7–0,8 с. Это ограниченная калибровка, не гарантия для любых полей. Подробности — [технический конспект](Backflat/DEVELOPMENT.md#fits-координаты-и-внешние-движки).

**Зависимости**: numpy, astropy, scipy; `diplib==3.6.1` для `fast`, pygame для окна. `--no-gui --median-mode exact` не требует последних двух. SxT и StarNet не входят в зависимости PULSAR, не скачиваются и не активируются скриптом. `--sxt` использует установленный лицензированный `rc-astro`, `--sxt-exe FILE` задаёт его путь. `--starnet` использует современный StarNet2 с FITS и `--linear` (2.6+), `--starnet-exe FILE` задаёт путь; старый TIFF-only StarNet++ отклоняется. См. [RC-Astro](https://www.rc-astro.com/) и [официальный CLI StarNet](https://starnetastro.com/documentation/starnet/command-line-tool/). Для движков выполняется обратимое линейное масштабирование общего RGB-диапазона, результат возвращается в исходные единицы. Основной режим `--starless` работает без обоих движков. Ориентация всех источников определяется описанной выше проверкой. Кэш хранит исходный порядок строк движка; новый ключ `backflat-starless-v3-raw-y` исключает старые форматы, включая `:sxt-flip-y-v1`. При каждом чтении выполняется та же проверка, поэтому флип не накапливается. Реальный StarNet2 в среде разработки не запускался; его адаптер и оба порядка строк проверены с подменённым движком.
...
Медианы и диффузия больших дыр могут занимать минуты на полноразмерном кадре. Пересчёт выполняется по Apply, окно остаётся отзывчивым. Быстрый режим не означает обработку в реальном времени. Технические выводы, измерения и ограничения: [Backflat/DEVELOPMENT.md](Backflat/DEVELOPMENT.md).
...
**Назначение**: Зеркальное отражение FITS по осям X и/или Y (пиксельный флип).

**ВАЖНО про термин.** «Флип по оси» в обиходе понимают по-разному и путают оси примерно в половине случаев. Здесь флип — это РАЗВОРОТ соответствующей координаты, определённый строго формулой:
- `--y` — флип по Y: `Ynew = H - Yold - 1` (разворот индекса строк) -> ВЕРХ <-> НИЗ (вертикальный переворот, картинка «вверх ногами»). Лево/право НЕ меняются.
- `--x` — флип по X: `Xnew = W - Xold - 1` (разворот индекса столбцов) -> ЛЕВО <-> ПРАВО (горизонтальное зеркало). Верх/низ НЕ меняются.

То есть «флип по Y» разворачивает координату Y (верх<->низ), а НЕ отражает «от линии оси Y» (это было бы лево<->право, т.е. `--x`). Если сомневаешься — смотри на формулу, а не на слова.

Оси можно комбинировать (`--x --y` = поворот на 180°). Значения пикселей и тип данных сохраняются точно (флип — обратимый геометрический пересчёт). Ключи WCS НЕ корректируются (флип только по пикселям). Поддержка 2D и 3-канальных (3×H×W или H×W×3) — отражаются только пространственные оси.
...
**Параметры**:
- `--y` — флип по Y: `Ynew = H - Yold - 1`, верх <-> низ (вертикальный переворот)
- `--x` — флип по X: `Xnew = W - Xold - 1`, лево <-> право (горизонтальное зеркало)
- нужна хотя бы одна ось; можно указать обе

**Примеры**:
```batch
:: Вертикальный переворот (верх <-> низ)
flip in.fit out.fit --y

:: Горизонтальное зеркало (лево <-> право), пакетно
flip *.fit flipped/ --x

:: Обе оси = поворот на 180 градусов
flip in.fit out.fit --x --y
```

**Зависимости**: только numpy/astropy (batch_utils).
...
**Назначение**: Создание масок для обработки из FITS-изображений.

Опциональный конвейер, в таком порядке:
1. цветное -> ЧБ по формуле `g = (R + 2*G + B) / 4` (только для 3-канального входа)
2. обрезка чёрной/белой точки + линейный стретч (`--black` / `--white`). Каждая граница задаётся в ОДНОМ из двух режимов:
   - **перцентиль** - если после числа стоит символ `%` (0..100): граница ставится на этом перцентиле яркости кадра. Например `--white 90%` - белая точка на 90-м перцентиле (самые яркие 10% уходят в белое).
   - **абсолютная яркость** - если `%` нет: число из диапазона 0..1 трактуется как доля насыщения формата. Насыщение = максимум типа данных для целочисленных кадров (65535 для uint16, 255 для uint8, ...) и 1.0 для float. Например `--black 0.01` - это 0.01 на float-кадре и `0.01*65535` на uint16-кадре.

   Режимы можно **смешивать** (например `--black 1% --white 0.95`). Обе границы вычисляются по ИСХОДНОМУ кадру, ДО стретча; затем диапазон [чёрная, белая] линейно растягивается на весь выходной диапазон (целые: [0, max типа]; float: [0, 1]). Белая точка, как и чёрная, отсчитывается от 0 (это абсолютный уровень, а не «верхние X %»). Значения по умолчанию - `--black 0 --white 1` (абсолютный режим): это истинный no-op, кадр НЕ меняется. Форма `--black 0% --white 100%` тоже ничего не «обрезает», но растягивает фактические min/max кадра на весь диапазон (min-max нормализация) - совпадает с исходным лишь когда кадр уже занимает весь диапазон. Достаточно указать любую одну границу - вторая примет значение по умолчанию. Вычисленная чёрная граница должна быть строго меньше белой, иначе - ошибка.
3. наращивание/сужение круговой апертурой (`--grow R`): R>0 - дилатация (звёзды растут, max-фильтр), R<0 - эрозия (min-фильтр); применяется ДО инверсии
4. инверсия (`--invert`): негатив, последним шагом

Выход всегда одноканальный. Тип данных входа сохраняется.

**Синтаксис**:
```
makemask.py input_spec output_spec [опции]
```

**Параметры**:
- `input_spec` - файл, маска (*.fit), нумерованные или @list.txt
- `output_spec` - файл, нумерованный шаблон или директория
- `--black LEVEL` - чёрная точка (уходит в 0). Два режима: `90%` - перцентиль (0..100); `0.01` - абсолютная яркость, доля насыщения формата (0..1)
- `--white LEVEL` - белая точка (уходит в насыщение), те же два режима. Любая из границ включает стретч; недостающая берёт значение по умолчанию (чёрная `0`, белая `1`). Режимы можно смешивать. Требуется: вычисленная чёрная < белой.
- `--grow R` - радиус круговой апертуры (px): R>0 наращивает (max), R<0 сужает (min); до инверсии
- `--invert` - негатив (последним шагом)

**Примеры**:
```batch
:: Смешанный режим: чёрная на абсолютном 0.01 (=0.01*насыщение), белая на 90-м перцентиле (яркие 10% -> белое)
makemask src.fit mask.fit --black 0.01 --white 90%

:: Смешанный режим: чёрная на 1-м перцентиле, белая на абсолютном 0.95 насыщения
makemask src.fit mask.fit --black 1% --white 0.95

:: Маска звёзд: изолировать яркое по перцентилям, нарастить, инвертировать (защита фона)
makemask light.fit starmask.fit --black 92% --white 99.8% --grow 3 --invert

:: Цветной кадр -> ЧБ маска
makemask rgb.fit mask.fit --black 50% --white 99%
```

**Примечание для float-кадров**: в абсолютном режиме насыщение float принимается за 1.0. Для линейных float-кадров с произвольным диапазоном используйте режим перцентилей (`--black 1% --white 99%`), а не абсолютные доли.

**Зависимости**: scipy (морфология).

---

### blend.py

**Назначение**: Совмещение двух FITS-изображений по маске непрозрачности.

Маска - это полутоновое изображение, значение которого задаёт непрозрачность (opacity) операнда в каждом пикселе. С нормированной маской m в [0, 1]:

```
output = source * (1 - m) + operand * m
```
- белый пиксель маски (m = 1) -> выход = операнд
- чёрный пиксель маски (m = 0) -> выход = исходник
- серый пиксель (m = 0.5) -> среднее арифметическое исходника и операнда

Нормировка маски: масштабируется в [0, 1] по СВОЕМУ полному диапазону - максимум типа для целочисленных масок (65535 для uint16, 255 для uint8, ...) и 1.0 для float (float-маска должна быть уже в [0, 1]). Цветная маска приводится к серому по `(R + 2*G + B) / 4`; нефинитные пиксели -> 0 (полностью исходник).

**Синтаксис**:
```
blend.py source output mask operand [--mtf [K]] [--invert] [-o N]
blend.py source output operand -o N [--mtf [K]] [--invert]      (без маски)
```

Итоговая непрозрачность `m` строится СТРОГО в этом порядке:
1. база - маска (нормированная в [0,1]) или, если маска не указана, ровная `1.0`;
2. `--mtf` - изменить полутона базовой непрозрачности;
3. `--invert` - `m -> 1 - m`;
4. `-o N` - умножить всё на `N` В САМОМ КОНЦЕ.

То есть `-o` только ослабляет ФИНАЛЬНУЮ непрозрачность («сделать всё как обычно, но на N силы»): с маской -> ослабленная маска (`m*N`), без маски -> ровное `N`.

**Параметры**:
- `source` - базовое изображение(я): файл, маска (*.fit), нумерованные или @list.txt
- `output` - файл, нумерованный шаблон или директория
- `mask` - полутоновая маска непрозрачности (FITS-файл или последовательность по числу кадров). НЕ числовая константа. Можно опустить ТОЛЬКО вместе с `-o/--opacity` (тогда берётся ровная непрозрачность).
- `operand` - изображение, подмешиваемое там, где непрозрачность выше: FITS-файл, последовательность или числовая константа (ровный уровень)
- `-o N`, `--opacity N` - умножить финальную непрозрачность на `N`, применяется В САМОМ КОНЦЕ (после `--mtf` и `--invert`). `N` - доля [0,1] или процент с суффиксом `%` (например `0.3` или `30%`); при отсутствии N по умолчанию `0.5`. С маской ослабляет её до N-силы; без маски (аргумент маски опущен) даёт ровную непрозрачность `N`.
- `--mtf [K]` - применить MTF к БАЗОВОЙ непрозрачности в [0,1], ДО `--invert` и ДО `-o`. K - баланс полутонов (0<K<1), как в mtf.py: K<0.5 повышает непрозрачность (больше операнда), K>0.5 понижает. По умолчанию 0.25. На ровной базе `1.0` (без маски) MTF не влияет - у неё нет полутонов.
- `--invert` - инвертировать непрозрачность (`m -> 1 - m`); ПОСЛЕ `--mtf`, ДО `-o`. Без маски инверсия ровной `1.0` даёт `0` (ничего не подмешивается).

**Примеры**:
```batch
:: Подмешать stars.fit поверх base.fit там, где mask.fit светлая
blend base.fit out.fit mask.fit stars.fit

:: Применить ту же маску на половину силы (mask * 0.5)
blend base.fit out.fit mask.fit stars.fit -o 50%

:: Ровное наложение stars.fit на 30% (без маски)
blend base.fit out.fit stars.fit -o 30%

:: Ровное среднее 50/50 двух изображений
blend base.fit out.fit hi.fit -o 0.5

:: Осветлить полутона маски перед совмещением (больше hi.fit)
blend base.fit out.fit mask.fit hi.fit --mtf 0.2

:: Совмещать там, где маска ТЁМНАЯ
blend base.fit out.fit mask.fit hi.fit --invert
```

`source` и `operand` должны совпадать по форме и шкале данных; выход сохраняет тип и заголовок `source`. Поддерживаются 2D и 3-канальные цветные изображения (моно-маска и ровная непрозрачность транслируются на все каналы). Если операнд - числовая константа, задавайте K для `--mtf` явно (например `--mtf 0.3`), чтобы константа не была принята за K.
...
**Назначение**: Выравнивание уже-выровненной-по-звёздам серии по ядру кометы.

Входные кадры уже совмещены по звёздам (общая пиксельная сетка). В pygame-окне вы отмечаете ядро кометы на ПЕРВОМ и ПОСЛЕДНЕМ кадрах (по времени съёмки); затем каждый кадр сдвигается по линейной интерполяции движения кометы во времени - комета остаётся неподвижной, звёзды смещаются. Результат готов к стекингу в кадр с резкой кометой.

**Синтаксис**:
```
cometalign.py input_spec output_spec [опции]
```

**Параметры**:
- `input_spec` - кадры, выровненные по звёздам (маска, нумерованные, @list.txt)
- `output_spec` - файл, нумерованный шаблон или директория
- `--start X Y` / `--stop X Y` - ядро кометы (px) на самом раннем / самом позднем кадре. Задайте ОБА, чтобы пропустить GUI (вы гарантируете, что координаты относятся к первому/последнему кадру по времени).
- `--ref first|last` - на каком кадре комета неподвижна (по умолчанию `first`)
- `--mtf M` - начальный стретч отображения (по умолчанию 0.05, меньше = ярче; только GUI)
- `--fill V` - значение для пикселей вне футпринта после сдвига (по умолчанию 0)

**Управление GUI** (хоткеи также показаны текстом в строке статуса):

| Клавиша | Действие |
|---------|----------|
| ЛКМ | поставить крестик на ядро кометы (текущий кадр) |
| Tab | переключить первый / последний кадр (пан/зум сохраняются) |
| Стрелки | двигать картинку (Shift = быстрее) |
| +/- или колесо | зум +/- (колесо зумит к курсору) |
| Home/End | ярче / темнее (MTF) |
| Enter | подтвердить (оба крестика заданы) и закрыть |
| Q/Escape | отмена (ничего не пишется) |

**Как работает**: время берётся из хедера (`JD` -> `MJD-OBS` -> `DATE-OBS`). По отметкам `p0` (первый, `t0`) и `p1` (последний, `t1`) каждый кадр времени `ti` сдвигается на `ref - (p0 + f*(p1-p0))`, где `f = (ti-t0)/(t1-t0)`, кубической субпиксельной интерполяцией; пиксели вне футпринта = `--fill`. Отметки сохраняются в sidecar `*.cometpts.csv` и печатаются готовой строкой `--start/--stop` для повторного запуска без GUI.

**Зависимости**: pygame (интерактивный режим), scipy (сдвиг).

**Примеры**:
```batch
:: Интерактивно: кликнуть ядро на первом, Tab, на последнем, Enter
cometalign aligned*.fit comet\c0001.fit

:: Без GUI (координаты заданы вручную)
cometalign aligned*.fit comet\c0001.fit --start 1520 980 --stop 1495 1012

:: Комета неподвижна относительно последнего кадра
