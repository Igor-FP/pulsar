...
- **Нулевые пиксели** (рамки нет-данных после совмещения) исключаются из подгонки во **всех** методах — иначе масса точек `(0, 0)` притягивает прямую к началу координат (leverage) и смещает offset `C` к нулю.
