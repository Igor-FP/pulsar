...
def usage():
    sys.stderr.write(
        "Usage:\n"
        "  mul.py input.fit output.fit operand [scale]\n"
        "\n"
...
def apply_mul_operation(base_data, operand, scale):
    """Core math: result = base * operand * scale, with dtype-safe clamping."""
...
def process_file(infile, outfile, operand_spec, file_index, scale):
    ...
    with fits.open(infile, memmap=False) as hdul:
        ...
        new_data = apply_mul_operation(data, operand, scale)
